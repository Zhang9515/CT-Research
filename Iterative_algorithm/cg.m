function [x,flag,resNE,iter] = cg(A,b,tol,maxit,prnt,x0,xstar)
% modified by ZXZ in 19/07/17
% this code is targeted on the symmetric matrix problem
...
if prnt
    head = '    k       x(1)             x(n)           residual        resNE';
    form = '%5.0f %16.10g %16.10g %13g %12.5g\n';
    disp('  ');   disp(head);
    fprintf(form, k, x(1), x(n), normr0, 1);
end
...
while (k < maxit) && (flag == 0)
    ...
    normx = norm(x);
    xmax  = max(xmax, normx);
    flag  = (normr <= normr0 * tol) || (normx * tol >= 1);
    
    % Output
    resNE = normr / normr0;
%     if prnt, fprintf(form, k, x(1), x(n), normr, resNE); end
    if prnt, fprintf(form, k, x(1), x(n), normr, norm(x-xstar)); end
end % while
