...
% try robust PCA
img = phantom(512);
patchmatrix = im2col(img,[8 ,8],'sliding');

load G:\CTcode\Data\IterativeFDK\Diskphantom\Display.mat   % result of FDK
testmatrix = squeeze(Display(64,:,:))';
pic = Diskphantom ( 128 ) ;     % true image
truematrix = squeeze(pic(64,:,:))';

[m,n]=size(testmatrix);
lambda = 0.05%1.0/sqrt(max(m,n));
mu = 10*lambda;
tol = 1e-6;
max_iter = 4000;
[L, S] = RobustPCA(testmatrix, lambda, mu, tol, max_iter);
